...
function char_inv(&$sqlr, &$sqlc)
{
    ...
    if ($sqlc->num_rows($result))
    {
        ...
        if (($user_lvl > $owner_gmlvl)||($owner_name === $user_name))
        {
            ...
            $result = $sqlc->query('SELECT ci.bag, ci.slot, ci.item, ii.count as stack_count, ii.itemEntry as item_template
                                    FROM character_inventory as ci INNER JOIN item_instance as ii on ii.guid = ci.item

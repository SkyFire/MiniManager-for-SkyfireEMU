                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name=\"new_expansion\">
                                            <option value=\"3\">{$lang_user['cata']}</option>
...
function edit_user()
{
    ...
    $refguid = $sqlm->fetch_assoc($sqlm->query('SELECT InvitedBy FROM mmfs_point_system_invites WHERE PlayersAccount = '.$data['id'].''));
    ...
    if ($sqlr->num_rows($result))
    {
        ...
        if($user_lvl >= $action_permission['update'])
        {
            ...
            foreach ($gm_level_arr as $level)
            {
                ...
                {
                   if (($data['name'] != $user_name) && ($level[0] > $user_lvl ))
                      continue;
                    $output .= "
...
function doupdate_referral($referredby, $user_id)
{
    ...
    $result = $sqlm->fetch_row($sqlm->query("SELECT InvitedBy FROM mmfs_point_system_invites WHERE PlayersAccount = '$user_id'"));
    $result = $result[0];

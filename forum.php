...
function forum_index(){
  ...
  $side = get_side();
    ...
    $result = $mysql->query("SELECT `authorname`,`id`,`name`,`time`,`forum` FROM `mmfs_forum_posts` WHERE `id` IN (SELECT MAX(`id`) FROM `mmfs_forum_posts` GROUP BY `forum`) ORDER BY `forum`;");
    $lasts = array();
      ...
      if($category["side_access"] != "ALL"){ // Not an all side forum
        ...
        if($forum["side_access"] != "ALL"){ // Not an all side forum
          if($side == "NO") // No char
            ...
            $totaltopics = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE forum = '$id' AND id = `topic`;");
            $numtopics = $mysql->num_rows($totaltopics);
            $totalreplies = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE forum = '$id';");
...
function forum_view_forum(){
  ...
  $topics = $mysql->query("SELECT id, authorid, authorname, name, annouced, sticked, closed FROM mmfs_forum_posts WHERE (forum = '$id' AND id = `topic`) OR annouced = 1 AND id = `topic` ORDER BY annouced DESC, sticked DESC, lastpost DESC LIMIT $start, $maxqueries;");
  $result = $mysql->query("SELECT `topic` as curtopic,(SELECT count(`id`)-1 FROM mmfs_forum_posts WHERE `topic` = `curtopic`) AS replies,lastpost as curlastpost,(SELECT authorname FROM mmfs_forum_posts WHERE id = curlastpost) as authorname,(SELECT time FROM mmfs_forum_posts WHERE id = curlastpost) as time FROM `mmfs_forum_posts` WHERE (`forum` = $id AND `topic` = `id` ) OR annouced = 1;");
  $lasts = array();
  if($mysql->num_rows($result) > 0){
    while($row = $mysql->fetch_row($result))
      $lasts[$row[0]] = $row;
  ...
  if($mysql->num_rows($topics)!=0){
    ...
    $totaltopics = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE forum = '$id' AND id = `topic`;"); //My page system is so roxing, i can' t break this query xD
...
function forum_view_topic(){
  ...
  if(!$post){
    $posts = $mysql->query("SELECT id,authorid,authorname,forum,name,text,time,annouced,sticked,closed FROM mmfs_forum_posts WHERE topic = '$id' ORDER BY id ASC LIMIT $start, $maxqueries;");
...
//    $link = $mysql->connect($realm_db['addr'], $realm_db['user'], $realm_db['pass'], $realm_db['name']);
    ...
    $totalposts = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE topic = '$id';");
    ...
    $post = $mysql->query("SELECT topic, id FROM mmfs_forum_posts WHERE id = '$id'"); // Get our post id
    ...
    $topic = $post[0];
    $posts = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE topic = '$topic';"); // Get posts in our topic
    $replies = $mysql->num_rows($posts);
    if($replies==0)
      error($forum_lang["no_such_topic"]);
    $row = 0;
...
function forum_do_edit_close(){
  ...
  $mysql->query("UPDATE mmfs_forum_posts SET closed = '$state' WHERE id = '$id'");
  $mysql->close();
  redirect("forum.php?action=view_topic&id=$id");
  // Queries : 1
}
function forum_do_edit_announce(){
  ...
  $mysql->query("UPDATE mmfs_forum_posts SET annouced = '$state' WHERE id = '$id'");
  $mysql->close();
  redirect("forum.php?action=view_topic&id=$id");
  // Queries : 1
}
function forum_do_edit_stick(){
  ...
  $mysql->query("UPDATE mmfs_forum_posts SET sticked = '$state' WHERE id = '$id'");
  $mysql->close();
  redirect("forum.php?action=view_topic&id=$id");
  // Queries : 1
}
function forum_delete_post(){
  ...
  $topic = $mysql->query("SELECT id,topic,authorid,forum FROM mmfs_forum_posts WHERE id = '$id';");
  ...
  $topic2 = $mysql->query("SELECT name FROM mmfs_forum_posts WHERE id = '{$topic[1]}';");
...
function forum_do_delete_post(){
  ...
  $topic = $mysql->query("SELECT id,topic,name,authorid,forum FROM mmfs_forum_posts WHERE id = '$id';");
  if($mysql->num_rows($topic)==0) error($forum_lang["no_such_post"]);
  $topic = $mysql->fetch_row($topic);
  ...
  if($id==$topic[1]){
    $mysql->query("DELETE FROM mmfs_forum_posts WHERE topic = '$id'");
    redirect("forum.php?action=view_forum&id=$fid");
  }
  else
  {
    $mysql->query("DELETE FROM mmfs_forum_posts WHERE id = '$id'");
    $result = $mysql->query("SELECT id FROM mmfs_forum_posts WHERE topic = '{$topic[1]}' ORDER BY id DESC LIMIT 1;"); // get last post id
    $lastpostid = $mysql->fetch_row($result);
    $lastpostid = $lastpostid[0];
    $mysql->query("UPDATE mmfs_forum_posts SET lastpost = '$lastpostid' WHERE id = '{$topic[1]}'"); // update topic' s last post id
...
function forum_add_topic(){
  ...
  if($minfloodtime > 0)
  {
    $userposts = $mysql->query("SELECT time FROM mmfs_forum_posts WHERE authorid = '$user_id' ORDER BY id DESC LIMIT 1;");
...
//  $msg = str_replace('\r', '<br />', $msg);
  ...
  $mysql->query("INSERT INTO mmfs_forum_posts (authorid, authorname, forum, name, text, time) VALUES ('$user_id', '$user_name', '$forum', '$name', '$msg', '$time');");
  $id = $mysql->insert_id();
  $mysql->query("UPDATE mmfs_forum_posts SET topic = '$id', lastpost = '$id' WHERE id = '$id';");
...
//  $msg = str_replace('\r', '<br />', $msg);
  ...
  $name = $mysql->query("SELECT name FROM mmfs_forum_posts WHERE id = '$topic';");
  $name = $mysql->fetch_row($name);
  $name = $mysql->quote_smart($name[0]);

  $time = date("m/d/y H:i:s");

  $mysql->query("INSERT INTO mmfs_forum_posts (authorid, authorname, forum, topic, name, text, time) VALUES ('$user_id', '$user_name', '$forum', $topic, '$name', '$msg', '$time');");
  $id = @mysql_insert_id($link);
  $mysql->query("UPDATE mmfs_forum_posts SET lastpost = $id WHERE id = $topic;");
...
function forum_edit_post(){
  ...
  $post = $mysql->query("SELECT id,topic,authorid,forum,name,text FROM mmfs_forum_posts WHERE id = '$id';");
...
//  $msg = str_replace('\r', '<br />', $msg);
  ...
  $result = $mysql->query("SELECT topic FROM mmfs_forum_posts WHERE id = $post;");
  $topicid = $mysql->fetch_row($result);

  $mysql->query("UPDATE mmfs_forum_posts SET text = '$msg' WHERE id = $post;");

  if($topic == 1){
    $mysql->query("UPDATE mmfs_forum_posts SET name = '$name' WHERE topic = {$topicid[0]};");
  }
...
function forum_move_topic(){
  ...
  $topic = $mysql->query("SELECT id,topic,authorid,forum, name FROM mmfs_forum_posts WHERE id = '$id';");
  //                0 1   2   3   4
...
function forum_do_move_topic(){
  ...
  $mysql->query("UPDATE mmfs_forum_posts SET forum = '$forum' WHERE topic = '$id'"); // update topic' s last post id
  redirect("forum.php?action=view_topic&id=$id");
  // Queries : 1
}

...
function guild_bank(&$sqlr, &$sqlc)
{
    ...
    if($sqlc->num_rows($result))
    {
        ...
        $result = $sqlc->query('SELECT gbi.SlotId, ii.itemEntry as item_entry, ii.count as stack_count FROM guild_bank_item gbi INNER JOIN item_instance ii on ii.guid = gbi.item_guid WHERE gbi.guildid = '.$guild_id.' AND TabID = '.$current_tab.'');
        $gb_slots = array();

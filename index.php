...
function front(&$sqlr, &$sqlc, &$sqlm)
{
    ...
    $output .= '
                    '.$lang_index['skyfire_rev'].' '.$version['core_revision'].' '.$lang_index['using_db'].' '.$version['db_version'].'
                </div>';
    unset($version);
    ...
    $all_record_m = $sqlm->result($sqlm->query('SELECT count(*) FROM mmfs_motd'), 0);
    ...
    if($all_record_m)
    {
        $result = $sqlm->query('SELECT id, realmid, type, content FROM mmfs_motd WHERE realmid = '.$realm_id.' ORDER BY id DESC LIMIT '.$start_m.', 3');
    ...
    if ($online)
    {
        ...
        else
        {
            ...
            if ($sqlc->num_rows($result))
                $order_side = (in_array($sqlc->result($result, 0),array(2,5,6,8,10,22))) ? ' AND race IN (2,5,6,8,10,22) ' : ' AND race IN (1,3,4,7,9,11) ';
        }
        if($order_by == 'ip')

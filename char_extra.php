...
function char_extra(&$sqlr, &$sqlc, &$sqlw)
{
    ...
    if ($sqlc->num_rows($result))
    {
        ...
        if (($user_lvl > $owner_gmlvl)||($owner_name === $user_name))
        {
            ...
            {
                $result_2 = $sqlc->query('SELECT ci.item, ii.itemEntry as item_template FROM item_instance as ii, character_inventory as ci WHERE ci.guid = '.$id.' AND ii.itemEntry = '.$bag['entry'].' ');
                while ($char = $sqlc->fetch_assoc($result_2))
                {
                    $result_3 = $sqlc->query('SELECT itemEntry AS item FROM item_instance WHERE guid = '.$char['item'].' ');
